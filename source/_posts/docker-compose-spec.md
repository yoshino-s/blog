...
# Compose Spec Compose 标准)

## 文档状态

本文档指定了用于定义多容器应用程序的 Compose 文件格式。 本文件的分发不受限制。

本文中的关键词 "MUST", "MUST NOT", "REQUIRED", "SHALL", "SHALL NOT", "SHOULD", "SHOULD NOT", "RECOMMENDED", "MAY", and "OPTIONAL" 遵循该 RFC 中的解释 [RFC 2119](https://tools.ietf.org/html/rfc2119)。
...
网络是允许服务相互通信的层。 暴露给服务的网络模型仅限于与目标服务和外部资源的简单 IP 连接，而网络定义允许微调平台提供的实际实现。

可以通过在顶层配置 `networks` 部分下指定网络名称来创建网络。
服务可以通过在服务的 [`networks`](#networks) 小节下指定网络名称来连接到网络。

在以下示例中，将在运行时创建网络 `front-tier` 和 `back-tier`，并且 `frontend` 服务会连接到`front-tier`网络和`back-tier`网络。
...
`driver` 指定应该为此网络使用哪个驱动程序。 如果驱动程序在平台上不可用，Compose 实现必须返回一个错误。

```yml
driver: overlay
```

默认值和可用值是特定于平台的。 Compose 规范必须支持以下的驱动程序：`none` 和 `host`
...
对于例如 `hosts` 和 `none` 的内建网络，因为这种网络的实现不包括在在 Compose 规范内，所以他们的格式是略有不同的。你必须使用 `host` 或者 `none`，或者使用别名作为名字定义一个外部网络，来让容器使用这些网络。
...
`driver_opts` 使用一个键值对的列表来指定网络驱动程序的配置选项。这些选项是特定于网络驱动程序的。详情可以参阅网络驱动程序的文档。这是一个可选项。
...
可以使用 [YAML anchor](http://www.yaml.org/spec/1.2/spec.html#id2765878) 复用配置片段。
...
在这个样例中，一个 名字叫 `default-volume` 的 _锚点_ 基于 名为 `db-data` 的卷的配置创建了。这个 _锚点_ 在之后以 `*default-volume` 的别名重用在卷 `metrics` 的定义中。同样的逻辑可以在 Compose 文件的任意元素使用。

> 锚点的解析发生在[变量插值](#Interpolation)之前，因此变量不能用于锚点的设置和别名。

使用
[YAML 合并类型](http://yaml.org/type/merge.html)，也可以实现部分覆盖由锚引用设置的值。在以下示例中，`metrics` 卷定义使用别名避免重复定义，但是覆盖 `name` 属性：
...
特殊扩展字段可以是任何格式，只要它们的名称以 `x-` 开头。 它们可以被使用在 Compose 文件中的任何结构中。 这是 Compose 实现静默忽略无法识别的字段的唯一例外。
...
Compose 规范未指定此类字段的内容，可用于启用自定义功能。 遇到未知扩展字段的实现时不会失败，但可以警告未知字段。

按平台/供应商名称，浏览器添加的方式相同
支持 [自定义 CSS 功能](https://www.w3.org/TR/2011/REC-CSS2-20110607/syndata.html#vendor-keywords)

对于平台扩展，建议以平台/供应商名称作为扩展前缀，与浏览器添加的方式相同
支持 [自定义 CSS 功能](https://www.w3.org/TR/2011/REC-CSS2-20110607/syndata.html#vendor-keywords)
